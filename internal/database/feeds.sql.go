...
const markFeedFeteched = `-- name: MarkFeedFeteched :one
UPDATE feeds
...
func (q *Queries) MarkFeedFeteched(ctx context.Context, id uuid.UUID) (Feed, error) {
	row := q.db.QueryRowContext(ctx, markFeedFeteched, id)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
